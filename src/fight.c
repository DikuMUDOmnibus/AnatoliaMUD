...
void one_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt ,bool secondary)
{
    ...
    if ( (result = skill_failure_nomessage(ch, gsn_deathblow, 0)) == 0
     ...
     * (Maybe it shouldn't print the message [too much flood])
    if ( result == 2 ) {
      skill_failure_check(ch, gsn_deathblow, 0, 0, NULL);
    }
    */
...
"You became a ghost permenantly and leave the earth realm.\n\r",
		    ...
		    victim->position = POS_STANDING;
                    sprintf( strsave, "%s/%s", ana_config.lib_player_dir,
