...
extern bool anatolia_down;
extern int  anatolia_exit;
...
*/
	...
	if (!check_parse_name(new_name))
	{
        ...
        sprintf( strsave, "%s/%s",
			ana_config.lib_player_dir, capitalize( new_name ) ); 
	...
	sprintf( strsave, "%s/%s",
			ana_config.lib_player_dir, capitalize( victim->name ) );
...
void reboot_anatolia( bool fMessage, int fType )
{
  DESCRIPTOR_DATA *d,*d_next;
  char buf[MAX_STRING_LENGTH];

    if ( fType == ANATOLIA_REBOOT ) {
      sprintf( log_buf, "Rebooting ANATOLIA.");
      sprintf( buf, "Anatolia is going down for reboot NOW!\n\r" );
    }
    else {
      sprintf( log_buf, "Shutting down ANATOLIA.");
      sprintf( buf, "Anatolia is going down for halt NOW!\n\r" );
    }
    log_string(log_buf);
    ...
    {
	d_next = d->next;
	if ( fMessage ) { write_to_buffer( d, buf, 0 ); }
        if ( d->character != NULL )
	{ 
